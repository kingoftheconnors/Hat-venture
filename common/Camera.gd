...
func _physics_process(delta):
	...
	if move_walls:
		if target_spot != Vector2.ZERO:
			...
			lim_left = min(get_camera_screen_center().x-(OptionsMenu.get_resolution().x/2), position.x-(OptionsMenu.get_resolution().x/2))
			lim_right = max(get_camera_screen_center().x+(OptionsMenu.get_resolution().x/2), position.x+(OptionsMenu.get_resolution().x/2))
		...
		# Enforce limits
		if position.y > lim_bottom - OptionsMenu.get_resolution().y/2:
			position.y = lim_bottom - OptionsMenu.get_resolution().y/2
		if position.y < lim_top + OptionsMenu.get_resolution().y/2:
			position.y = lim_top + OptionsMenu.get_resolution().y/2
		if position.x > lim_right - (OptionsMenu.get_resolution().x/2):
			position.x = lim_right - (OptionsMenu.get_resolution().x/2)
		if position.x < lim_left + (OptionsMenu.get_resolution().x/2):
			position.x = lim_left + (OptionsMenu.get_resolution().x/2)
...
func room_capture(spot, is_move_walls):
	if target_spot != spot:
		...
		tween.start()
		lim_bottom = spot.y + OptionsMenu.get_resolution().y/2 + (OptionsMenu.get_resolution().y/2) * drag_margin_bottom
...
func reset_limits():
	lim_top = level.up - (OptionsMenu.get_resolution().y/2) * drag_margin_top
	lim_bottom = level.down + (OptionsMenu.get_resolution().y/2) * drag_margin_bottom
	lim_left = level.left - (OptionsMenu.get_resolution().x/2) * drag_margin_left
	lim_right = level.right + (OptionsMenu.get_resolution().x/2) * drag_margin_right
